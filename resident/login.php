...
if (isset($_SESSION['residentlogin']) && $_SESSION['residentlogin'] == "GatedResidentLoggedin") {
...
if (isset($_POST['submitcheck'])) {
    ...
    $stmt = mysqli_prepare($conn, "SELECT `id`, `name`, `email`, `mobile`, `pass`, `members`, `fourwheeler`, `twowheeler`, `status`, `block`, `housenum`, `date_time` FROM gated_resident WHERE email = ?");
    ...
    mysqli_stmt_bind_result($stmt, $id, $name, $email, $mobile, $hashed_password, $members, $fourwheeler, $twowheeler, $status, $block, $housenum, $date_time);
    ...
    if (mysqli_stmt_fetch($stmt)) {
        ...
        if (password_verify($input_password, $hashed_password)) {
            ...
            $_SESSION['residentName'] = $name;
            $_SESSION['residentEmail'] = $email;
            $_SESSION['residentMobile'] = $mobile;
            $_SESSION['residentMembers'] = $members;
            $_SESSION['residentFourWheeler'] = $fourwheeler;
            $_SESSION['residentTwoWheeler'] = $twowheeler;
            $_SESSION['residentStatus'] = $status;
            $_SESSION['residentBlock'] = $block;
            $_SESSION['residentHouseNum'] = $housenum;
            $_SESSION['residentDateTime'] = $date_time;
            $_SESSION['residentlogin'] = "GatedResidentLoggedin";  // Use this for checking if the user is logged in
